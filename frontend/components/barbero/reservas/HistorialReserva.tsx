import type { HistorialReserva as HistorialReservaItem } from "@/lib/types";
...
export function HistorialReserva({ historial }: { historial: HistorialReservaItem[] }) {
  return (
    <section className="rounded-xl border border-[#d4af37]/20 bg-[#0d0d0d] p-5 shadow-xl shadow-black/20">
      <h2 className="text-xl font-black text-white">Historial de la reserva</h2>
      {historial.length === 0 ? (
        <div className="mt-4 rounded-lg border border-white/10 bg-[#171717] p-5 text-center text-sm text-[#b5b5b5]">
          Aún no hay historial disponible para esta reserva.
        </div>
      ) : (
        <div className="mt-4 grid gap-3">
          {historial.map((item) => (
            <article
              ...
              key={item.idHistorialEstadoReserva ?? `${item.estadoNuevo}-${item.fechaCreacion}`}
            >
              ...
              <p className="mt-2 text-sm leading-6 text-[#b5b5b5]">
                {item.comentario ?? item.motivo ?? "Sin observación registrada."}
              </p>
